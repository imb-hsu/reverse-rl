...
def collectData(experiment_name="RPPO_BW_then_FW_25"):
    ...
    #return everything
    return timesteps, mean_rewards, mean_ep_lengths, std_rewards, std_ep_lengths
...
def drawPlots(experiment_names = ["PPO_Forward_Baseline_5", "PPO_Forward_Baseline_10"]):
    ...
    for experiment_name in experiment_names:
        ...
        timesteps, mean_rewards, mean_ep_lengths, std_rewards, std_ep_lengths = collectData(experiment_name)
        # plot training data
        ...
        plt.savefig('./Logging/' + experiment_name +'_rewards.png')
        plt.close()
    ...
    for experiment_name in experiment_names:
        ...
        # plot training data
        #plt.subplot(1, len(experiment_names), i)
        plt.plot(timesteps, mean_ep_lengths)
        plt.fill_between(timesteps, mean_ep_lengths - std_ep_lengths, mean_ep_lengths + std_ep_lengths, alpha=0.2)
        ...
        #plt.style.use('fivethirtyeight')
        # plt.show() # commented out on remote pc
        plt.savefig('./Logging/' + experiment_name +'_timesteps.png')
        plt.close()

def main():
    #drawPlots(["PPO_Forward_Baseline_5", "PPO_Forward_Baseline_10", "PPO_Forward_Baseline_25"])
    drawPlots(["RPPO_BW_Then_FW_5","RPPO_BW_Then_FW_10"])
